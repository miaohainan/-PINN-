"""
Multi-scale Fourier Feature PINN for 2D Poisson Equation (JAX)
===============================================================
Generalize to 2D Poisson Equation: -\Delta u(x,y) = f(x,y)
...
RESULTS_DIR = os.path.join(WORKDIR, f'results_2d_{TIMESTAMP}')
...
# PDE parameters
# ============================================================
# 修改：针对二维多尺度问题，适当调整高频频率（B_FREQ），同时维持 sigma_ff 设置
A_FREQ = 2
B_FREQ = 10 
SIGMA_FF = 5.0

def u_exact(x):
    # 修改：构造二维多频率源项对应的精确解 (结合不同方向和不同尺度)
    # u(x,y) = sin(A*pi*x)*sin(A*pi*y) + 0.1*sin(B*pi*x)*sin(B*pi*y)
    return (np.sin(np.pi * A_FREQ * x[:, 0]) * np.sin(np.pi * A_FREQ * x[:, 1]) + 
            0.1 * np.sin(np.pi * B_FREQ * x[:, 0]) * np.sin(np.pi * B_FREQ * x[:, 1]))

def laplacian_exact(x):
    # 修改：2维 Poisson 方程 -\Delta u = f，计算 f(x,y) = u_xx + u_yy
    u_xx = (-(np.pi * A_FREQ)**2 * np.sin(np.pi * A_FREQ * x[:, 0]) * np.sin(np.pi * A_FREQ * x[:, 1])
            - 0.1 * (np.pi * B_FREQ)**2 * np.sin(np.pi * B_FREQ * x[:, 0]) * np.sin(np.pi * B_FREQ * x[:, 1]))
    u_yy = (-(np.pi * A_FREQ)**2 * np.sin(np.pi * A_FREQ * x[:, 0]) * np.sin(np.pi * A_FREQ * x[:, 1])
            - 0.1 * (np.pi * B_FREQ)**2 * np.sin(np.pi * B_FREQ * x[:, 0]) * np.sin(np.pi * B_FREQ * x[:, 1]))
    return u_xx + u_yy
...
def apply_nn(params, x_norm):
    # 修改：由于变成了2维坐标，输入维度需 reshape 为 (1, 2)
    H = x_norm.reshape(1, 2)
    for (W, b) in params[:-1]:
        H = jnp.tanh(H @ W + b)
    W, b = params[-1]
    return (H @ W + b)[0, 0]
...
# ============================================================
# PDE residual: Laplacian (\Delta u)
# ============================================================
# 修改：重写求导函数，2维 PDE 需使用 Hessian 矩阵来求偏导的和 (u_xx + u_yy)
def make_laplacian_fn(apply_fn_single, sigma_X):
    def laplacian_single(*args_and_x):
        *all_args, x_n = args_and_x
        def u_of_x(xn):
            return apply_fn_single(*all_args, xn)
        H = jax.hessian(u_of_x)(x_n)
        # 各自方向求导结果需使用对应方向的 sigma 进行缩放恢复物理尺度
        return H[0, 0] / (sigma_X[0] ** 2) + H[1, 1] / (sigma_X[1] ** 2)
...
def build_loss_fns_nn_ff(apply_fn, sigma_X, frozen_args):
    # 修改：传递完整的 sigma_X 数组以适应 x, y 双方向计算
    u_xx_fn = make_laplacian_fn(apply_fn, sigma_X)
    ...
    # 修改：增加对二维边界四条边的 loss_bc 计算
    def loss_bc(params, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4):
        pred1 = u_pred_batch(params, x_bc1)
        pred2 = u_pred_batch(params, x_bc2)
        pred3 = u_pred_batch(params, x_bc3)
        pred4 = u_pred_batch(params, x_bc4)
        return (jnp.mean((pred1 - u_bc1) ** 2) + jnp.mean((pred2 - u_bc2) ** 2) + 
                jnp.mean((pred3 - u_bc3) ** 2) + jnp.mean((pred4 - u_bc4) ** 2))

    def loss_res(params, x_r, f_r):
        uxx = u_xx_batch(params, x_r)
        return jnp.mean((uxx - f_r) ** 2)

    def loss_total(params, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4, x_r, f_r):
        l_bc = loss_bc(params, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4)
...
def build_loss_fns_mff(sigma_X):
    ...
    # 修改：支持四条边的 BC loss
    def loss_bc(ps, wf, W1, W2, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4):
        pred1 = u_pred_batch(ps, wf, W1, W2, x_bc1)
        pred2 = u_pred_batch(ps, wf, W1, W2, x_bc2)
        pred3 = u_pred_batch(ps, wf, W1, W2, x_bc3)
        pred4 = u_pred_batch(ps, wf, W1, W2, x_bc4)
        return (jnp.mean((pred1 - u_bc1) ** 2) + jnp.mean((pred2 - u_bc2) ** 2) + 
                jnp.mean((pred3 - u_bc3) ** 2) + jnp.mean((pred4 - u_bc4) ** 2))
    ...
    def loss_total(ps, wf, W1, W2, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4, x_r, f_r):
        l_bc = loss_bc(ps, wf, W1, W2, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4)
...
def train_nn_ff(model_name, apply_fn, params, frozen_args, n_frozen,
    ...
    # 修改：传递四条边界条件
    @jit
    def train_step(params, opt_state, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4, x_r, f_r):
        (loss_val, (l_bc, l_r)), grads = jax.value_and_grad(
            lambda p: loss_total_fn(p, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4, x_r, f_r),
            has_aux=True)(params)
    ...
    def predict_u(params, X_pts):
        ...
        # 修改：去除 X_norm[:, 0]，因网络已适配 2D 输入
        return np.array(u_pred_batch(params, jnp.array(X_norm)))

    # 修改：构造二维测试网格以计算全区域精确的 L2 误差
    nn_test = 100
    xx, yy = np.meshgrid(np.linspace(0, 1, nn_test), np.linspace(0, 1, nn_test))
    X_test_2d = np.column_stack([xx.flatten(), yy.flatten()])
    u_test_2d = u_exact(X_test_2d)

    # 修改：为了兼容原代码的1D曲线画图结构，在测试集中提取 y=0.5 的截面，使得画图函数能直接无缝执行
    X_test_slice = np.column_stack([np.linspace(0, 1, 1000), np.full(1000, 0.5)])
    u_test_slice = u_exact(X_test_slice)

    # 修改：生成用于 NTK 的 2D 点
    np.random.seed(seed)
    X_ntk_raw = np.random.rand(ntk_n_pts, 2)
    X_ntk_norm = (X_ntk_raw - mu_X) / sigma_X
    x_ntk = jnp.array(X_ntk_norm)
    ...
    for it in range(n_iter):
        ...
        X_bc3, u_bc3 = bcs_samplers[2].sample(batch_size, rng_train)
        X_bc4, u_bc4 = bcs_samplers[3].sample(batch_size, rng_train)
        X_res, f_res = res_sampler.sample(batch_size, rng_train)

        X_bc1_n = (X_bc1 - mu_X) / sigma_X
        X_bc2_n = (X_bc2 - mu_X) / sigma_X
        X_bc3_n = (X_bc3 - mu_X) / sigma_X
        X_bc4_n = (X_bc4 - mu_X) / sigma_X
        X_res_n = (X_res - mu_X) / sigma_X

        # 修改：传入四个边界数据
        params, opt_state, loss_val, l_bc, l_r = train_step(
            params, opt_state,
            jnp.array(X_bc1_n), jnp.array(u_bc1.flatten()),
            jnp.array(X_bc2_n), jnp.array(u_bc2.flatten()),
            jnp.array(X_bc3_n), jnp.array(u_bc3.flatten()),
            jnp.array(X_bc4_n), jnp.array(u_bc4.flatten()),
            jnp.array(X_res_n), jnp.array(f_res.flatten()))

        if it % log_every == 0:
            # 修改：在计算相对 L2 Error 时使用全区域测试集，客观反映 2D 效果
            u_pred_2d = predict_u(params, X_test_2d)
            l2_err = np.linalg.norm(u_test_2d - u_pred_2d) / np.linalg.norm(u_test_2d)
            ...
            print(f"  It: {it:5d}, Loss: {float(loss_val):.3e}, "
                  ...
                  f"L2 (2D): {l2_err:.3e}, Time: {elapsed:.1f}s")
    ...
    # 修改：画图返回的数据使用 1D 截面 y=0.5
    u_pred_slice = predict_u(params, X_test_slice)
    u_pred_final_2d = predict_u(params, X_test_2d)
    final_l2 = np.linalg.norm(u_test_2d - u_pred_final_2d) / np.linalg.norm(u_test_2d)
    ...
    return {
        ...
        # 修改：使得 X_test 维持为 1D 以兼容后续所有的绘图函数
        'u_pred_final': u_pred_slice, 'u_test': u_test_slice,
        'X_test': X_test_slice[:, 0], 'params': params,
    }
...
def train_mff(model_name, params_shared, W_final_tuple, W1, W2,
    ...
    @jit
    def train_step(trainable, opt_state, x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4, x_r, f_r):
        ps, wf = trainable
        (loss_val, (l_bc, l_r)), grads = jax.value_and_grad(
            lambda tr: loss_total_fn(tr[0], tr[1], W1, W2,
                                     x_bc1, u_bc1, x_bc2, u_bc2, x_bc3, u_bc3, x_bc4, u_bc4, x_r, f_r),
    ...
    def predict_u(trainable, X_pts):
        ps, wf = trainable
        X_norm = (X_pts - mu_X) / sigma_X
        return np.array(u_pred_batch(ps, wf, W1, W2, jnp.array(X_norm)))

    # 修改：同样为 mFF 生成二维评估集以及 y=0.5 的画图一维截面
    nn_test = 100
    xx, yy = np.meshgrid(np.linspace(0, 1, nn_test), np.linspace(0, 1, nn_test))
    X_test_2d = np.column_stack([xx.flatten(), yy.flatten()])
    u_test_2d = u_exact(X_test_2d)
    ...
    for it in range(n_iter):
        ...
        if it % log_every == 0:
            u_pred_2d = predict_u(trainable, X_test_2d)
            l2_err = np.linalg.norm(u_test_2d - u_pred_2d) / np.linalg.norm(u_test_2d)
            loss_bc_log.append(float(l_bc))
            loss_res_log.append(float(l_r))
            l2_error_log.append(float(l2_err))
    ...
    total_time = time.time() - start_time
    u_pred_slice = predict_u(trainable, X_test_slice)
    u_pred_final_2d = predict_u(trainable, X_test_2d)
    final_l2 = np.linalg.norm(u_test_2d - u_pred_final_2d) / np.linalg.norm(u_test_2d)
    ...
    return {
        ...
        'ntk_K_log': ntk_K_log, 'ntk_iters_log': ntk_iters_log,
        'u_pred_final': u_pred_slice, 'u_test': u_test_slice,
        'X_test': X_test_slice[:, 0], 'params': trainable,
    }
...
# ============================================================
# Single model plots (保持原样 - 无需改动，复用了截面数据)
# ============================================================
...
def save_comparison_table(all_res):
    lines = ["=" * 80,
             "COMPARISON TABLE — 2D Poisson Equation (Multi-scale PINN)",
             "=" * 80,
...
def main():
    print("=" * 70)
    print("Multi-scale Fourier Feature PINN — 2D Poisson Equation")
    print(f"a={A_FREQ}, b={B_FREQ}, sigma={SIGMA_FF}")
    print(f"u(x,y) = sin({A_FREQ}*pi*x)sin({A_FREQ}*pi*y) + 0.1*sin({B_FREQ}*pi*x)sin({B_FREQ}*pi*y)")
    ...
    # 修改：构建2维问题的采样器域边界
    bc1_sampler = Sampler(2, np.array([[0.0, 0.0], [0.0, 1.0]]), lambda x: u_exact(x), name='BC_x0')
    bc2_sampler = Sampler(2, np.array([[1.0, 0.0], [1.0, 1.0]]), lambda x: u_exact(x), name='BC_x1')
    bc3_sampler = Sampler(2, np.array([[0.0, 0.0], [1.0, 0.0]]), lambda x: u_exact(x), name='BC_y0')
    bc4_sampler = Sampler(2, np.array([[0.0, 1.0], [1.0, 1.0]]), lambda x: u_exact(x), name='BC_y1')
    res_sampler = Sampler(2, np.array([[0.0, 0.0], [1.0, 1.0]]), lambda x: laplacian_exact(x), name='Residual')
    bcs_samplers = [bc1_sampler, bc2_sampler, bc3_sampler, bc4_sampler]
    ...
    # 修改：同理输入维度升至 2
    W1 = random.normal(k1, (2, 50)) * 1.0
    W2 = random.normal(k2, (2, 50)) * SIGMA_FF
